...
struct RealEstateForm: View {
    ...
    // Naming
    @AppStorage("vektor.finance.re.address")  private var address: String = ""

    // Property
    @AppStorage("vektor.finance.re.price")    private var purchasePrice: Double = 500_000
    @AppStorage("vektor.finance.re.closing")  private var closingCostsPercent: Double = 3.0
    @AppStorage("vektor.finance.re.currency") private var currency: String = "USD"
    // Financing
    @AppStorage("vektor.finance.re.downPct")  private var downPaymentPercent: Double = 25
    @AppStorage("vektor.finance.re.rate")     private var mortgageRatePercent: Double = 6.5
    @AppStorage("vektor.finance.re.term")     private var loanTermYears: Double = 30
    // Income
    @AppStorage("vektor.finance.re.rent")     private var monthlyRent: Double = 3_500
    @AppStorage("vektor.finance.re.vacancy")  private var vacancyPercent: Double = 5
    @AppStorage("vektor.finance.re.other")    private var otherMonthlyIncome: Double = 0
    @AppStorage("vektor.finance.re.rentGrow") private var annualRentGrowthPercent: Double = 3
    // OpEx
    @AppStorage("vektor.finance.re.tax")      private var propertyTaxAnnual: Double = 4_500
    @AppStorage("vektor.finance.re.ins")      private var insuranceAnnual: Double = 1_200
    @AppStorage("vektor.finance.re.maint")    private var maintenancePercentOfRent: Double = 8
    @AppStorage("vektor.finance.re.mgmt")     private var propertyMgmtPercentOfRent: Double = 8
    @AppStorage("vektor.finance.re.hoa")      private var hoaMonthly: Double = 0
    @AppStorage("vektor.finance.re.capex")    private var capExPercentOfRent: Double = 5
    @AppStorage("vektor.finance.re.util")     private var utilitiesAnnual: Double = 0
    // Hold + exit
    @AppStorage("vektor.finance.re.appr")     private var appreciationPercent: Double = 3
    @AppStorage("vektor.finance.re.hold")     private var holdYears: Double = 10
    @AppStorage("vektor.finance.re.sellPct")  private var sellingCostsPercent: Double = 6
            ...
            Chart {
                ForEach(r.yearByYear, id: \.year) { yr in
                    ...
                    .foregroundStyle(yr.cashFlow >= 0
                        ? VektorTheme.accent.opacity(0.7)
                        : VektorTheme.statusBad.opacity(0.7))
                }
            }
            .frame(height: 160)
        }
        ...
        return Section {
            Chart {
                ForEach(r.yearByYear, id: \.year) { yr in
                    ...
                    .foregroundStyle(VektorTheme.accent.opacity(0.30))
