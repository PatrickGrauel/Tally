...
///
/// Consumers (`AppModel`) listen for `.vektorNetworkReconnected` and react —
...
public final class Reachability: @unchecked Sendable {
    ...
    public static let reconnectedNotification = Notification.Name("vektor.network.reconnected")

    private let monitor: NWPathMonitor
    private let queue = DispatchQueue(label: "app.vektor.Vektor.reachability")
    private let lock = NSLock()
    private var _lastSatisfied: Bool? = nil
    private static let logger = Logger(subsystem: "app.vektor.Vektor", category: "reachability")
